...
**Document Version**: 2.0 (Railway Compatible)  
**Date**: October 5, 2025  
...
**⚠️ Updated for Railway Backend Migration**: This version reflects Bearer token authentication for cross-domain compatibility.
...
SigmaSight uses a **Bearer token authentication system** with centralized token management for Railway backend compatibility.

### Core Components

1. **Centralized Authentication**
   - `authManager.ts` - JWT token management (localStorage)
   - `chatAuthService.ts` - Auth service using Bearer tokens
...
3. **Backend Integration**
   ...
   - Railway backend at `https://sigmasight-be-production.up.railway.app`
   - JWT tokens in Authorization headers (Bearer token)
   - Works cross-domain (localhost ↔ Railway)
...
│ - Uses Bearer token from authManager │
...
### 3. Chat Auth Service Processes Login (Railway Compatible)
...
#### Step 3.3: Store Session Data (Centralized)

```typescript
// Store via authManager (centralized token management)
authManager.setSession({
...
#### Step 3.4: Resolve Portfolio ID (Bearer Token)
...
#### Step 3.5: Initialize Conversation (Bearer Token)

```typescript
try {
  // Uses authManager.getAccessToken() for Bearer token
...
### 5. Portfolio Resolver Discovers Portfolio ID (Bearer Token)
...
async getUserPortfolioId(forceRefresh = false): Promise<string | null> {
  // Get token from authManager (centralized)
...
#### Step 5.2: Fetch from Backend (Bearer Token)
...
1. **Primary**: Fetch from backend API (using Bearer token)
...
### Parallel API Fetching (Bearer Token)
...
async function fetchPortfolioDataFromApis(
  ...
  const [overviewResult, positionsResult, factorExposuresResult] =
    await Promise.allSettled([
      ...
      // 2. Positions API - position details (Bearer token)
      apiClient.get(`/api/v1/data/positions/details?portfolio_id=${portfolioId}`, {
        headers: { Authorization: `Bearer ${token}` },
        signal: abortSignal
      }),

      // 3. Factor Exposures API - factor data (Bearer token)
...
const logout = useCallback(async () => {
  try {
    // 1. Call backend logout endpoint (with Bearer token)
...
### Chat Auth Service Logout (Bearer Token)
...
async logout(): Promise<void> {
  try {
    ...
    // Clear backend session (with Bearer token)
    await fetch(`${this.baseUrl}/api/v1/auth/logout`, {
      method: 'POST',
      headers: token ? { 'Authorization': `Bearer ${token}` } : {},
      // NO credentials: 'include' - Bearer only
    });
  } catch (error) {
...
### 2. chatAuthService (Bearer Token Auth)

**File**: `frontend/src/services/chatAuthService.ts`

**Purpose**: Authentication service using Bearer tokens (Railway compatible)
...
// Conversation Management (Bearer token)
...
- **Bearer Tokens Only**: All requests use `Authorization: Bearer {token}`
- **No Cookies**: Removed `credentials: 'include'` for Railway compatibility
- **Centralized Tokens**: All token access via `authManager.getAccessToken()`
...
```typescript
// Portfolio Discovery (Bearer token)
...
- **Backend Fetch**: On cache miss or force refresh (uses Bearer token)
...
3. If token exists → Validate with backend (Bearer token)
...
| `services/chatAuthService.ts` | Bearer Token Auth | 409 |
...
| `/api/v1/auth/logout` | POST | User logout | Bearer Token |
| `/api/v1/auth/me` | GET | Get current user | Bearer Token |
| `/api/v1/data/portfolios` | GET | List user portfolios | Bearer Token |
| `/api/v1/data/portfolio/{id}/complete` | GET | Full portfolio data | Bearer Token |
| `/api/v1/data/positions/details` | GET | Position details | Bearer Token |
| `/api/v1/analytics/portfolio/{id}/factor-exposures` | GET | Factor exposures | Bearer Token |
| `/api/v1/chat/conversations` | POST | Create conversation | Bearer Token |
| `/api/v1/chat/send` | POST | Send chat message (SSE) | Bearer Token |
...
1. **Login** → chatAuthService.login() (Bearer token)
2. **Store JWT** → authManager.setSession()
3. **Resolve Portfolio ID** → portfolioResolver.getUserPortfolioId() (Bearer token)
4. **Store Portfolio ID** → Zustand portfolioStore
5. **Navigate** → /portfolio page
6. **Load Data** → usePortfolioData() hook
7. **Fetch APIs** → portfolioService.loadPortfolioData() (Bearer tokens)
...
- **All API Auth**: Bearer tokens only (no cookies)

### Key Design Decisions

1. **Bearer Token Only**: No cookies - works cross-domain with Railway
2. **Centralized Token Management**: All token access via authManager
3. **No In-App Portfolio Switching**: Each login = one portfolio session
4. **Aggressive Caching**: 5-minute memory cache, localStorage persistence
5. **Parallel API Calls**: Promise.allSettled for graceful degradation
6. **Zustand for Portfolio ID**: Global state with built-in persistence
7. **React Context for Auth**: Standard pattern for authentication state
...
### Railway Migration Updates

**Version 2.0 changes for Railway compatibility**:
- ✅ Removed all HttpOnly cookie dependencies
- ✅ All authentication uses Bearer tokens
- ✅ Centralized token access through authManager
- ✅ Works cross-domain (localhost ↔ Railway)
- ✅ Removed `credentials: 'include'` from all fetch calls
- ✅ Updated SSE streaming to use Bearer tokens

---

**Document Maintained By**: SigmaSight Engineering Team  
**Last Updated**: October 5, 2025  
**Related Docs**: `project-structure.md`, `API_AND_DATABASE_SUMMARY.md`, `RAILWAY_MIGRATION.md`
