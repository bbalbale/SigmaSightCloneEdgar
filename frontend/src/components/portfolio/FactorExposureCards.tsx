...
const getBetaColor = (beta: number) => {
  ...
  return 'text-secondary bg-primary border-primary'
}

// Skeleton card for loading state
const SkeletonCard = () => (
  <div className="bg-white rounded-lg border border-primary p-2 animate-pulse">
...
const FactorCard = ({ factor }: { factor: FactorExposure }) => {
  ...
  return (
    <div className={`rounded-lg border p-2 transition-all duration-200 hover:shadow-md cursor-pointer ${colorClass}`}>
      <div className="text-xs font-medium text-secondary mb-0.5">
        {factor.name}
      </div>
      ...
      <div className="container mx-auto">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-3">
            <h3 className="text-sm font-medium text-secondary">Factor Exposures</h3>
            <DataQualityIndicator dataQuality={dataQuality} />
          </div>
          <span className="text-xs text-tertiary">Beta values and dollar exposures</span>
        </div>
