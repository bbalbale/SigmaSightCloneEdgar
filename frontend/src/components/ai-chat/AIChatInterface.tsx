 ...
 *
 * Chat UI for AI Insights on SigmaSight AI page.
 * Uses OpenAI Responses API via the provider-agnostic aiChatService.
 */
...
export function AIChatInterface() {
  ...
  const handleFeedback = async (message: AIChatMessage, rating: 'up' | 'down') => {
    ...
    if (!message.backendMessageId || message.role !== 'assistant') {
      console.warn('Cannot submit feedback: no backend message ID')
      return
    }
        ...
        {messages.map((message) => (
          <div key={message.id} className="space-y-2">
            ...
            >
              ...
              <div className="flex items-center justify-between mt-2 pt-2 border-t transition-colors duration-300" style={{
                borderColor: 'var(--border-primary)'
              }}>
                {/* Tool calls count */}
                <div className="text-xs" style={{ color: 'var(--text-tertiary)' }}>
                  {message.tool_calls_count && message.tool_calls_count > 0 && (
                    <span>Used {message.tool_calls_count} analytics tool{message.tool_calls_count > 1 ? 's' : ''}</span>
                  )}
                </div>
                ...
                {message.role === 'assistant' && message.backendMessageId && (
                  <div className="flex items-center gap-1">
                    <button
                      ...
                      style={{
                        color: message.feedback === 'up' ? 'var(--color-success, #10b981)' : 'var(--text-tertiary)'
                      }}
                    ...
                    <button
                      ...
                      style={{
                        color: message.feedback === 'down' ? 'var(--color-error, #ef4444)' : 'var(--text-tertiary)'
                      }}
                      title="Not helpful"
                    >
