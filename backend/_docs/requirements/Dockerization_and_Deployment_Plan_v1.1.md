# Dockerization and Deployment Plan v1.2
...
- Create Railway Dev Postgres (v15) and enable scheduled backups; copy `DATABASE_URL`.
- Put `DATABASE_URL=...&sslmode=require` in `backend/.env.local`; require SSL in engine (`connect_args={"ssl":"require"}`, `pool_pre_ping=True`).
- Run Alembic against Dev DB: `alembic upgrade head`; verify schema and seed once if needed.
- Configure a single-writer daily OHLCV Cron Job on Railway; upsert by `(symbol, date)`; log to `etl_run`.
- Keep FE/BE running locally with hot reload; point only the DB to Railway.
- Add a simple safety: require `CONFIRM_REMOTE_DB=1` env for destructive scripts/migrations.
...
- Backend (FastAPI)
  ...
  - Start command (Railway): `python -m uvicorn app.main:app --host 0.0.0.0 --port $PORT`.
...
- Migrations: Alembic is the only path for schema changes. Run via Railway shell or pre‑start step.
...
Proposed next steps
1. Create Railway Dev project: Postgres + Backend (Nixpacks) with `/health` and env vars.
