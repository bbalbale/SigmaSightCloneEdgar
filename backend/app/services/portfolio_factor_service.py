...
# Comparison utility for dual-write validation
async def compare_symbol_vs_position_factors(
    db: AsyncSession,
    portfolio_id: UUID,
    calculation_date: date
) -> Dict[str, Any]:
    """
    Compare symbol-level aggregation vs position-level calculation.

    This is used during the transition period to validate that
    symbol-level factors match position-level calculations.

    Args:
        db: Database session
        portfolio_id: Portfolio to compare
        calculation_date: Date to compare
    ...
    logger.info(f"Comparing symbol vs position factors for {portfolio_id}")

    # Get symbol-level aggregation
    symbol_result = await get_portfolio_factor_exposures(
        db, portfolio_id, calculation_date, include_ridge=True, include_spread=True
    )

    # Get position-level calculation (existing method)
    position_ridge_result = await calculate_factor_betas_ridge(
        db, portfolio_id, calculation_date
    )
    position_spread_result = await calculate_portfolio_spread_betas(
        db, portfolio_id, calculation_date
    )
    ...
    # Compare Ridge factors
    symbol_ridge = symbol_result.get('ridge_betas', {})
    position_ridge = position_ridge_result.get('factor_betas', {})

    for factor_name in set(symbol_ridge.keys()) | set(position_ridge.keys()):
        sym_val = symbol_ridge.get(factor_name, 0.0)
        pos_val = position_ridge.get(factor_name, 0.0)
        diff = abs(sym_val - pos_val)

        comparison['ridge_comparison'][factor_name] = {
            'symbol_level': sym_val,
            'position_level': pos_val,
            'difference': diff,
            'match': diff < 0.01  # Tolerance
        }

        comparison['max_ridge_diff'] = max(comparison['max_ridge_diff'], diff)
        if diff >= 0.01:
            comparison['match'] = False

    # Compare Spread factors
    symbol_spread = symbol_result.get('spread_betas', {})
    position_spread = position_spread_result.get('factor_betas', {})

    for factor_name in set(symbol_spread.keys()) | set(position_spread.keys()):
        sym_val = symbol_spread.get(factor_name, 0.0)
        pos_val = position_spread.get(factor_name, 0.0)
        diff = abs(sym_val - pos_val)

        comparison['spread_comparison'][factor_name] = {
            'symbol_level': sym_val,
            'position_level': pos_val,
            'difference': diff,
            'match': diff < 0.01
        ...
        comparison['max_spread_diff'] = max(comparison['max_spread_diff'], diff)
        if diff >= 0.01:
            comparison['match'] = False

    logger.info(
        f"Comparison complete: match={comparison['match']}, "
        f"max_ridge_diff={comparison['max_ridge_diff']:.4f}, "
        f"max_spread_diff={comparison['max_spread_diff']:.4f}"
    )
